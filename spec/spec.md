...
- 「まとめて読む」ボタンがあり、それを押すと日記まとめ読みページへ遷移する
...
- 日記プレビューページの「まとめて読む」ボタンから遷移する
- 縦スクロールで前後合わせて5日分の日記を表示する
  - 初期表示では、遷移元の日付の日記位置までスクロールされている
- スクロールに応じて前後の日付の日記を追加取得する無限スクロールになっている
- 各日付について、日記本文・未作成・取得エラーのいずれかを表示する
- 左矢印アイコンおよびスワイプ操作で日記プレビューページへ戻ることができる
...
- CodeMirror互換のマークダウン編集支援機能をもつ（JetBrains Markdownパーサーを使用）
  - 箇条書きリスト自動継続: `- `, `* `, `+ `で始まる行で改行すると、同じマーカーで次の行が継続される
  - 番号付きリスト自動継続: `1. `や`1) `で始まる行で改行すると、番号をインクリメントして次の行が継続される
  - チェックリスト自動継続: `- [ ] `や`- [x] `で始まる行で改行すると、未チェック状態`- [ ] `で次の行が継続される
  - ブロッククォート自動継続: `> `で始まる行で改行すると、`> `が次の行にも挿入される（ネスト`> > `も対応）
  - インデント保持: インデント付きリストの改行時に、同じインデントレベルを維持する
  - ブロッククォート内リスト: `> - `や`> 1. `の組み合わせでも正しく継続される
  - 空マーカー行でのリスト終了:
    - 最初の（唯一の）空項目: マーカー削除
    - タイトリストの2番目の空項目: 空行を挿入して非タイトリストに変換（マーカー保持）
    - 非タイトリストの空項目: マーカー削除
    - 3番目以降の空項目: マーカー削除
    - ブロッククォート内でも同じルール適用（`>`プレフィックス付き空行を挿入）
  - フェンスドコードブロック内無効化: `` ``` ``で囲まれたコードブロック内では、マークアップ継続が無効化される
...
- カレンダーページの検索アイコンボタンから遷移する
- GitHub Code Search APIを使用して、リポジトリ内の日記コンテンツをキーワード検索できる
- 検索ボックスに入力し、検索ボタンまたはEnterキーで検索を実行する
- 検索結果はファイルパスごとにテキストフラグメント付きで表示される
- 日記パス形式（`yyyy/MM/dd/README.md`）の結果をクリックすると日記プレビューページへ遷移する
- 月間目標パス形式（`yyyy/MM/README.md`）の結果はクリック不可（日付解析不能のため）
- 「さらに読み込む」ボタンによるページネーションをサポートする
- 左矢印アイコンおよびスワイプ操作でカレンダーページに戻ることができる
- 設定が未構成の場合は空の結果を返す
